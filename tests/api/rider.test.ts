...
describe("GET /api/rider/rides", () => {
  ...
  it("should return rider wallet details and transactions", async () => {
    await User.findByIdAndUpdate(riderId, { walletBalance: 250 });
    await Transaction.create({
      ...
      balanceBefore: 0,
      balanceAfter: 250,
      description: "Initial rider wallet credit",
      status: "completed",
    });
    ...
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(res.body.data.walletBalance).toBe(250);
...
describe("GET /api/rider/rides/:rideId", () => {
  it("should return 404 for non-existent ride", async () => {
    ...
    expect(res.body.success).toBe(false);
    console.log(
      `[GET /api/rider/rides/:rideId not found] status=${res.status} time=${time}ms`,
    );
  });
