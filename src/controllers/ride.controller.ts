...
import { calculateFare, calculateDistance, validateCoupon } from '../services/fare.service';
...
export async function bookRide(req: Request, res: Response): Promise<void> {
  ...
  const existingRide = await Ride.findOne({
    riderId: req.user!.id,
    status: { $in: ['searching', 'driver_assigned', 'driver_arrived', 'otp_verified', 'in_progress'] },
  });
  if (existingRide) {
    sendConflict(res, 'You already have an active ride');
    return;
  }
  ...
  // Find nearby drivers and broadcast ride request
  try {
    const nearbyDrivers = await (Driver as typeof Driver & { findNearby: Function }).findNearby(
      pickup.lat,
      pickup.lng,
    ...
    for (const driver of nearbyDrivers) {
      if (driver.socketId) {
        io.to(driver.socketId).emit('ride:new_request', {
          rideId: ride._id,
          pickup: { ...pickup, distance: distanceKm },
          drop,
          fare: fareBreakdown.finalFare,
          vehicleType,
    ...
    logger.info(`Ride ${ride._id} broadcast to ${nearbyDrivers.length} drivers`);
  } catch (err) {
    logger.error('Error broadcasting ride request:', err);
  }

  sendCreated(res, 'Ride booked. Searching for driver...', {
    rideId: ride._id,
    otp,
    fareBreakdown,
    status: 'searching',
...
export async function getRide(req: Request, res: Response): Promise<void> {
  const ride = await Ride.findById(req.params.rideId)
    .populate('driverId', 'name phone avatar vehicleType vehicleNumber vehicleColor rating location')
 ...
 */
export async function driverArrived(req: Request, res: Response): Promise<void> {
 ...
 */
export async function verifyRideOTP(req: Request, res: Response): Promise<void> {
...
export async function completeRide(req: Request, res: Response): Promise<void> {
  ...
  if (driver) {
    ...
    driver.walletBalance = Math.max(0, driver.walletBalance - ride.platformFee) + ride.driverEarning;
...
export async function cancelRide(req: Request, res: Response): Promise<void> {
  ...
  if (role === 'rider') {
    // Notify driver
    const driver = await Driver.findById(ride.driverId).select('socketId').lean();
    if (driver?.socketId) {
...
export async function rateRide(req: Request, res: Response): Promise<void> {
  ...
  const ride = await Ride.findOne({
    _id: req.params.rideId,
    status: 'completed',
    ...(role === 'rider' ? { riderId: req.user!.id } : { driverId: req.user!.id }),
  });
 ...
 */
export async function getActiveRide(req: Request, res: Response): Promise<void> {
  const role = req.user!.role;
  const query: Record<string, unknown> = {
    status: { $in: ['searching', 'driver_assigned', 'driver_arrived', 'otp_verified', 'in_progress'] },
  };
  ...
  const ride = await Ride.findOne(query)
    .populate('driverId', 'name phone avatar vehicleType vehicleNumber vehicleColor rating location socketId')
    .populate('riderId', 'name phone avatar rating')
    .lean();
 ...
 */
export async function getFareEstimate(req: Request, res: Response): Promise<void> {
  const { pickupLat, pickupLng, dropLat, dropLng, vehicleType, couponCode } =
    req.query as Record<string, string>;

  if (!pickupLat || !pickupLng || !dropLat || !dropLng || !vehicleType) {
    sendError(res, 'Missing required query params: pickupLat, pickupLng, dropLat, dropLng, vehicleType', 400);
    return;
  }
  ...
  const fareBreakdown = calculateFare(distanceKm, vehicleType as VehicleType, couponCode);
  sendSuccess(res, 'Fare estimate calculated', { distanceKm, ...fareBreakdown });
}
