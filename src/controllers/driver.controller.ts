...
export async function registerDriver(req: Request, res: Response): Promise<void> {
  ...
  sendSuccess(res, 'Registration submitted. Your account is pending verification.', {
...
export async function getProfile(req: Request, res: Response): Promise<void> {
  const driver = await Driver.findById(req.user!.id).select('-fcmToken -socketId').lean();
  if (!driver) {
...
export async function updateProfile(req: Request, res: Response): Promise<void> {
  const allowedFields = ['name', 'email', 'gender', 'dob', 'avatar', 'serviceArea'];
...
export async function getRideHistory(req: Request, res: Response): Promise<void> {
  ...
  const [rides, total] = await Promise.all([
    Ride.find({ driverId: req.user!.id })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .populate('riderId', 'name phone avatar rating')
      .lean(),
    Ride.countDocuments({ driverId: req.user!.id }),
  ]);
...
export async function getWallet(req: Request, res: Response): Promise<void> {
  ...
  const driver = await Driver.findById(req.user!.id).select('walletBalance totalEarnings').lean();
  if (!driver) {
    sendNotFound(res, 'Driver not found');
    return;
  }
